...
from polynomials_on_simplices.piecewise_polynomial.piecewise_polynomial_lagrange_basis import (
    PiecewisePolynomialLagrange, dual_piecewise_polynomial_lagrange_basis, generate_inverse_local_to_global_map,
    generate_local_to_global_map, piecewise_polynomial_lagrange_basis, unit_piecewise_polynomial_lagrange,
    zero_piecewise_polynomial_lagrange)
...
def create_scalar_valued_univariate_piecewise_polynomial():
    ...
    return PiecewisePolynomialLagrange(coeff, lines, vertices, r)
...
def create_scalar_valued_bivariate_piecewise_polynomial():
    ...
    return PiecewisePolynomialLagrange(coeff, triangles, vertices, r)
...
class TestUnivariateLagrangeBasis:
    ...
    def test_basis():
        ...
        for r in range(4):
            ...
            basis = piecewise_polynomial_lagrange_basis(lines, vertices, r, tau)
            dual_basis = dual_piecewise_polynomial_lagrange_basis(lines, vertices, r, tau, num_dofs, tau_inv)
...
class TestBivariateLagrangeBasis:
    ...
    def test_basis():
        ...
        for r in range(4):
            ...
            basis = piecewise_polynomial_lagrange_basis(triangles, vertices, r, tau)
            dual_basis = dual_piecewise_polynomial_lagrange_basis(triangles, vertices, r, tau, num_dofs, tau_inv)
            for i in range(num_dofs):
                for j in range(num_dofs):
...
def test_zero_piecewise_polynomial():
    ...
    lines = line_strip_primitives(n)
    vertices = np.linspace(0, 1, n)
    p = zero_piecewise_polynomial_lagrange(lines, vertices)
    assert p.basis() == "Lagrange"
    x = np.random.rand()
    assert abs(p(x)) < 1e-15
    for n in [2, 3]:
        p = zero_piecewise_polynomial_lagrange(lines, vertices, n)
        assert p.basis() == "Lagrange"
    ...
    p = zero_piecewise_polynomial_lagrange(triangles, vertices)
    assert p.basis() == "Lagrange"
    x = np.random.rand(2)
    assert abs(p(x)) < 1e-15
    for n in [2, 3]:
        p = zero_piecewise_polynomial_lagrange(triangles, vertices, n)
        assert p.basis() == "Lagrange"
        assert np.linalg.norm(p(x)) < 1e-15
...
def test_unit_piecewise_polynomial():
    ...
    lines = line_strip_primitives(n)
    vertices = np.linspace(0, 1, n)
    p = unit_piecewise_polynomial_lagrange(lines, vertices)
    assert p.basis() == "Lagrange"
    x = np.random.rand()
    assert abs(p(x) - 1) < 1e-15
    for n in [2, 3]:
        p = unit_piecewise_polynomial_lagrange(lines, vertices, n)
        assert p.basis() == "Lagrange"
    ...
    triangles = rectangle_triangulation(n, n)
    vertices = unit_square_vertices(n, n)[:, 0:2]
    p = unit_piecewise_polynomial_lagrange(triangles, vertices)
    assert p.basis() == "Lagrange"
    x = np.random.rand(2)
    assert abs(p(x) - 1) < 1e-15
    for n in [2, 3]:
        p = unit_piecewise_polynomial_lagrange(triangles, vertices, n)
        assert p.basis() == "Lagrange"
        assert np.linalg.norm(p(x) - np.ones(n)) < 1e-15
