...
from polynomials_on_simplices.piecewise_polynomial.continuous_piecewise_polynomial_bernstein_basis import (
    ContinuousPiecewisePolynomialBernstein, continuous_piecewise_polynomial_bernstein_basis,
    dual_continuous_piecewise_polynomial_bernstein_basis, generate_local_to_global_map,
    generate_local_to_global_preimage_map, unit_continuous_piecewise_polynomial_bernstein,
    zero_continuous_piecewise_polynomial_bernstein)
...
def create_scalar_valued_univariate_continuous_piecewise_polynomial():
    ...
    return ContinuousPiecewisePolynomialBernstein(coeff, lines, vertices, r)
...
def create_scalar_valued_bivariate_continuous_piecewise_polynomial():
    ...
    return ContinuousPiecewisePolynomialBernstein(coeff, triangles, vertices, r)
...
def test_call():
    ...
    expected_value_2 = 3.7777777777777777
    ...
    expected_value_2 = np.array([3.7777777777777777, 3.7777777777777777])
...
def test_call_on_boundary():
    ...
    p = ContinuousPiecewisePolynomialBernstein(coeff, lines, vertices, r, boundary_simplices=[[0]])
    assert p(0) == 0
    ...
    coeff = [4, 5, 6, 2, 1, 5]
    p = ContinuousPiecewisePolynomialBernstein(coeff, triangles, vertices, r, boundary_simplices=[[0, 1]])
    assert p((-0.5, -0.5)) == 0
    assert p((0.0, -0.5)) == 0
    assert p((0.5, -0.5)) == 0
    ...
    p = ContinuousPiecewisePolynomialBernstein(coeff, triangles, vertices, r, boundary_simplices=[[2, 3]],
                                               keep_boundary_dofs_last=True)
    assert p((-0.5, 0.5)) == -1
    assert p((0.0, 0.5)) == -1
    assert p((0.5, 0.5)) == -1
...
class TestUnivariateBernsteinBasis:
    ...
    def test_basis():
        ...
        for r in range(1, 4):
            ...
            basis = continuous_piecewise_polynomial_bernstein_basis(lines, vertices, r, tau)
            dual_basis = dual_continuous_piecewise_polynomial_bernstein_basis(lines, vertices, r, tau, num_dofs,
                                                                              tau_preim)
...
class TestBivariateBernsteinBasis:
    ...
    def test_basis():
        ...
        for r in range(1, 4):
            ...
            basis = continuous_piecewise_polynomial_bernstein_basis(triangles, vertices, r, tau)
            dual_basis = dual_continuous_piecewise_polynomial_bernstein_basis(triangles, vertices, r, tau, num_dofs,
                                                                              tau_preim)
            for i in range(num_dofs):
                for j in range(num_dofs):
...
def test_zero_continuous_piecewise_polynomial():
    ...
    lines = line_strip_primitives(n)
    vertices = np.linspace(0, 1, n)
    p = zero_continuous_piecewise_polynomial_bernstein(lines, vertices)
    assert p.basis() == "Bernstein"
    x = np.random.rand()
    assert abs(p(x)) < 1e-15
    for n in [2, 3]:
        p = zero_continuous_piecewise_polynomial_bernstein(lines, vertices, n)
        assert p.basis() == "Bernstein"
    ...
    p = zero_continuous_piecewise_polynomial_bernstein(triangles, vertices)
    assert p.basis() == "Bernstein"
    x = np.random.rand(2)
    assert abs(p(x)) < 1e-15
    for n in [2, 3]:
        p = zero_continuous_piecewise_polynomial_bernstein(triangles, vertices, n)
        assert p.basis() == "Bernstein"
        assert np.linalg.norm(p(x)) < 1e-15
...
def test_unit_continuous_piecewise_polynomial():
    ...
    lines = line_strip_primitives(n)
    vertices = np.linspace(0, 1, n)
    p = unit_continuous_piecewise_polynomial_bernstein(lines, vertices)
    assert p.basis() == "Bernstein"
    x = np.random.rand()
    assert abs(p(x) - 1) < 1e-15
    for n in [2, 3]:
        p = unit_continuous_piecewise_polynomial_bernstein(lines, vertices, n)
        assert p.basis() == "Bernstein"
    ...
    triangles = rectangle_triangulation(n, n)
    vertices = unit_square_vertices(n, n)[:, 0:2]
    p = unit_continuous_piecewise_polynomial_bernstein(triangles, vertices)
    assert p.basis() == "Bernstein"
    x = np.random.rand(2)
    assert abs(p(x) - 1) < 1e-15
    for n in [2, 3]:
        p = unit_continuous_piecewise_polynomial_bernstein(triangles, vertices, n)
        assert p.basis() == "Bernstein"
        assert np.linalg.norm(p(x) - np.ones(n)) < 1e-15
